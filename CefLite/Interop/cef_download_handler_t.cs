...
namespace CefLite.Interop
{
    ...
    public unsafe class CefDownloadHandler : ObjectFromNet<cef_download_handler_t, CefDownloadHandler>
    {
        ...
        static DelegateHolder<func_on_before_download> holder_on_before_download = new DelegateHolder<func_on_before_download>(
            (IntPtr self, cef_browser_t* browser, cef_download_item_t* item, cef_string_t* suggested_name, cef_before_download_callback_t* callback) =>
            {
                ...
                inst.BeforeDownload?.Invoke(inst, browser, item, cef_string_t.ToString(suggested_name), callback);
            });
        ...
        static DelegateHolder<func_on_download_updated> holder_on_download_updated = new DelegateHolder<func_on_download_updated>(
            ...
            {
                var inst = GetInstance(self);
                inst.DownloadUpdated?.Invoke(inst, browser, item, callback);
            });
    ...
    public unsafe class CefDownloadItem : ObjectFromCef<cef_download_item_t, CefDownloadItem>
    {
        private CefDownloadItem(IntPtr ptr) : base(ptr) { }
        static public CefDownloadItem FromNative(cef_download_item_t* ptr)
            => FromNative((IntPtr)ptr, (p2) => new CefDownloadItem(p2));
        public cef_download_item_t* FixedPtr => (cef_download_item_t*)Ptr;

        static public implicit operator CefDownloadItem(cef_download_item_t* ptr) => FromNative(ptr);
    ...
    public unsafe class CefBeforeDownloadCallback : ObjectFromCef<cef_before_download_callback_t, CefBeforeDownloadCallback>
    {
        private CefBeforeDownloadCallback(IntPtr ptr) : base(ptr) { }
        static public CefBeforeDownloadCallback FromNative(cef_before_download_callback_t* ptr)
            => FromNative((IntPtr)ptr, (p2) => new CefBeforeDownloadCallback(p2));
        public cef_before_download_callback_t* FixedPtr => (cef_before_download_callback_t*)Ptr;

        static public implicit operator CefBeforeDownloadCallback(cef_before_download_callback_t* ptr) => FromNative(ptr);
    ...
    public unsafe class CefDownloadItemCallback : ObjectFromCef<cef_download_item_callback_t, CefDownloadItemCallback>
    {
        private CefDownloadItemCallback(IntPtr ptr) : base(ptr) { }
        static public CefDownloadItemCallback FromNative(cef_download_item_callback_t* ptr)
            => FromNative((IntPtr)ptr, (p2) => new CefDownloadItemCallback(p2));
        public cef_download_item_callback_t* FixedPtr => (cef_download_item_callback_t*)Ptr;

        static public implicit operator CefDownloadItemCallback(cef_download_item_callback_t* ptr) => FromNative(ptr);
