...
namespace CefLite.Interop
{
    ...
    public unsafe class CefProcessMessage : ObjectFromCef<cef_process_message_t, CefProcessMessage>
    {
        private CefProcessMessage(IntPtr ptr) : base(ptr) { }
        static public CefProcessMessage FromNative(cef_process_message_t* ptr)
            => FromNative((IntPtr)ptr, (p2) => new CefProcessMessage(p2));
        static public CefProcessMessage FromNative(IntPtr ptr)
            => FromNative(ptr, (p2) => new CefProcessMessage(p2));
        public cef_process_message_t* FixedPtr => (cef_process_message_t*)Ptr;

        static public implicit operator CefProcessMessage(cef_process_message_t* ptr) => FromNative(ptr);
        ...
        {
            return CefListValue.FromNative(Marshal.GetDelegateForFunctionPointer<GetObjectHandler>(FixedPtr->get_argument_list)(Ptr));
        }
